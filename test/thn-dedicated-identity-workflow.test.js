...
test("THN identity release is manual, TEST-only and checks the source before AWS credentials", () => {
  ...
  assert.match(source, /options: \[verify, diagnose, inspect, apply\]/);
  assert.match(source, /failed_source_sha:/);
  assert.match(source, /git rev-list --first-parent -n 5 "\$first_parent" \| grep -Fqx "\$THN_FAILED_SOURCE_SHA"/);
