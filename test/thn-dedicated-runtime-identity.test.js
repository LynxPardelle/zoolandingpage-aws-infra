...
test("execution role may attach only the two policies generated by the reviewed SAM template", () => {
  const policy = namedPolicy(synth("test"), "ThnDedicatedRuntimeTestExecutionV1");
  const attach = policy.Properties.PolicyDocument.Statement.find(value =>
    value.Action.includes("iam:AttachRolePolicy"));
  assert.deepEqual(attach.Action, ["iam:AttachRolePolicy", "iam:DetachRolePolicy"]);
  const policyArns = attach.Condition.ArnEquals["iam:PolicyARN"].map(value => {
  ...
  assert.deepEqual(policyArns, [
    "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole",
    "arn:aws:iam::aws:policy/AWSXrayWriteOnlyAccess",
  ]);
  assert.ok(JSON.stringify(attach.Resource).includes("zoolanding-thn-auth-runti*"));
  assert.ok(!JSON.stringify(synth("production")).includes("AWSXrayWriteOnlyAccess"));
});
