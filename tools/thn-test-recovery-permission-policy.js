...
function revisionAction(original, service) {
  ...
  if (service === "hub-version") {hub.composeTemplate(original); return "Modify";}
  const selected = target(service);
  if (service === "auth-enable" && original?.Resources?.[selected.logical]) {
    if (!same(original.Resources[selected.logical], authEnable.previousPolicyResource())
      || Object.entries(authEnable.parameterDefinitions()).some(([name, definition]) => !same(original.Parameters?.[name], definition))) fail();
    return "Modify";
  }
...
function roleSnapshot(input, service, account, newDocument, previousDocument) {
  ...
  if (!keys(input, ["Role", "inline", "attached"]) || role?.RoleName !== selected.role
    ...
    || !object(input.inline) || !Object.keys(input.inline).length || Object.hasOwn(input.inline, policyName(service)) !== (modify && !managed)
    || !Array.isArray(input.attached)
    || !(managed && modify ? same(input.attached, [{PolicyName: selected.policyName, PolicyArn: authEnable.policyArn(account)}]) : input.attached.length === 0)
