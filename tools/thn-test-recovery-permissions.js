    ...
    try {
      ...
      if (service === "s3api") {
        if (action === "head-object") {
          if (["auth-provision", "image-version", "hub-version"].includes(config.service) || !keys(input, ["Bucket", "Key", "VersionId", "ExpectedBucketOwner"]) || input.ExpectedBucketOwner !== config.account
