...
const { createThnTestDeployIdentities } = require("./thn-test-deploy-identities");
const { createThnTestSupplementalPolicies } = require("../../tools/thn-test-permission-policy");
...
class ServiceRepositoryBootstrapStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    createThnTestDeployIdentities(this, environment);
    createThnTestSupplementalPolicies(this, environment);
    addCanonicalPolicy(this, environment, "auth-provision");
    addCanonicalPolicy(this, environment, "auth-enable");
    if (environment.name === "test") {
      // Existing services attach their closed mediator-invoke policies separately.
      // Preserve this logical resource and UsePreviousValue for its NoEcho parameter.
      const principal = new cdk.CfnParameter(this, "ThnTestHumanOperatorPrincipalArn", { type: "String", noEcho: true });
      const operator = new cdk.CfnResource(this, "ThnTestHumanOperatorRole", { type: "AWS::IAM::Role", properties: {
        RoleName: "zoolanding-thn-registry-test-operator",
        AssumeRolePolicyDocument: { Version: "2012-10-17", Statement: [{ Effect: "Allow", Principal: { AWS: principal.valueAsString }, Action: "sts:AssumeRole" }] },
      } });
      operator.overrideLogicalId("ThnTestHumanOperatorRole");
      operator.applyRemovalPolicy(cdk.RemovalPolicy.RETAIN);
    }
  }
}
