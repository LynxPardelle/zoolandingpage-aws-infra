...
function createBackendSamDeployRoles(scope, environment) {
  if (!["dev", "test"].includes(environment.name)) {
    return [];
  }
  ...
  const services = [
    {
      id: "ConfigAuthoringBackendDeployRole",
      roleName: `zoolanding-config-authoring-${deployEnvironment}-deploy`,
      repository: "zoolanding-config-authoring",
      stackName: `zoolanding-config-authoring-${deployEnvironment}`,
      buckets: [`zoolanding-config-payloads-${deployEnvironment}`],
      tables: [`zoolanding-config-registry-${deployEnvironment}`],
    },
    {
      id: "DataDropperBackendDeployRole",
      roleName: `zoolanding-data-dropper-${deployEnvironment}-deploy`,
      repository: "zoolanding-data-dropper-lambda",
      stackName: `zoolanding-data-dropper-${deployEnvironment}`,
      buckets: [`zoolanding-data-raw-${deployEnvironment}`],
      tables: [],
    },
  ];
