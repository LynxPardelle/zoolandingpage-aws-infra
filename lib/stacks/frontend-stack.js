...
function createViewerHostHeaderFunction(scope, environment, frontDoor, idSuffix) {
  ...
  const canonicalRedirectHost = frontDoor.redirectAlternateDomainNamesToPrimary
    ? JSON.stringify(frontDoor.domainName)
    : '""';
  const redirectAlternateHosts = frontDoor.redirectAlternateDomainNamesToPrimary
    ? JSON.stringify(frontDoor.alternateDomainNames || [])
    : "[]";
  ...
  var canonicalRedirectHost = ${canonicalRedirectHost};
  var redirectAlternateHosts = ${redirectAlternateHosts};
  if (canonicalRedirectHost && redirectAlternateHosts.indexOf(viewerHost) !== -1) {
    var queryParts = [];
