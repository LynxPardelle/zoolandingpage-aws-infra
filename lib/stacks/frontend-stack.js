...
class FrontendStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const frontDoors = frontend.frontDoors && frontend.frontDoors.length > 0 ? frontend.frontDoors : [frontend];
...
function createViewerHostHeaderFunction(scope, environment, frontDoor, idSuffix) {
  ...
  request.headers["x-forwarded-port"] = { value: "443" };
  request.headers["x-forwarded-server"] = { value: "cloudfront" };
  return request;
}`),
  });
}
