...
const { isHashedStaticAssetPath, validateSelection } = require("../../tools/thn-admin-release");
...
class FrontendStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    // Preserve the same prerequisite logical resource independently of front-door activation.
    // Normal release preflight must prove it already exists; this is not an issuance path.
    if (frontend.thnAdminCertificate) {
      if (environment.name !== "test") throw new Error("THN certificate ownership is TEST only.");
      const certificate = new cdk.CfnResource(this, "ThnAdminTestCertificate", {
        type: "AWS::CertificateManager::Certificate", properties: {
          ...
          DomainValidationOptions: [{ DomainName: "admin-test.thehairnarrative.com", HostedZoneId: frontend.thnAdminCertificate.hostedZoneId }],
        },
      });
      certificate.overrideLogicalId("ThnAdminTestCertificate");
      certificate.applyRemovalPolicy(cdk.RemovalPolicy.RETAIN);
      // The retained live certificate has no CDK path metadata. Keep its
      // template entry byte-for-byte stable during admin front-door activation.
      delete certificate.cfnOptions.metadata?.["aws:cdk:path"];
    }
...
function createThnAdminSsr(scope, environment, frontend, frontDoor, artifactBucket) {
  const releaseId = frontDoor.staticRelease.metadata.releaseId;
  const functionName = buildResourceName(environment, "frontend", "thn-admin-ssr");
  const logGroup = new logs.LogGroup(scope, "FrontendThnAdminSsrLogGroup", {
    logGroupName: `/aws/lambda/${functionName}`,
    retention: logs.RetentionDays.ONE_MONTH,
    removalPolicy: removalPolicyForEnvironment(environment),
  });
  const ssrFunction = new lambda.Function(scope, "FrontendThnAdminSsrFunction", {
    functionName,
    description: "Isolated The Hair Narrative TEST Journal SSR.",
    runtime: runtimeFromConfig(frontend.ssrRuntime),
    architecture: lambda.Architecture.ARM_64,
    handler: "index.handler",
    code: lambda.Code.fromBucket(artifactBucket,
      `${frontDoor.staticRelease.originPrefix}/server/ssr-handler.zip`),
    memorySize: frontend.ssrMemorySizeMb || 512,
    timeout: cdk.Duration.seconds(frontend.ssrTimeoutSeconds || 15),
    environment: {
      CONFIG_API_URL: frontend.apiBaseUrl,
      CONFIG_API_SERVER_FALLBACK_URL: frontend.configApiServerFallbackUrl || "",
      NG_ALLOWED_HOSTS: [frontDoor.domainName, `*.lambda-url.${environment.region}.on.aws`].join(","),
      NG_TRUST_PROXY_HEADERS: "x-forwarded-host,x-forwarded-proto,x-forwarded-for,x-forwarded-port",
      NODE_ENV: "production",
      ZLP_DEPLOY_ENV: "test",
      ZLP_RELEASE_ID: releaseId,
      ZLP_RUNTIME_ENV: "test",
    },
  });
  ssrFunction.node.addDependency(logGroup);
  const functionUrl = ssrFunction.addFunctionUrl({ authType: lambda.FunctionUrlAuthType.AWS_IAM });
  const ssrOrigin = origins.FunctionUrlOrigin.withOriginAccessControl(functionUrl, {
    readTimeout: cdk.Duration.seconds(frontend.ssrTimeoutSeconds || 15),
    keepaliveTimeout: cdk.Duration.seconds(5),
  });
  return { ssrFunction, ssrOrigin };
}
...
function createFrontendDistribution(scope, props) {
  ...
  const privateSsr = isThnAdminFrontDoor
    ? createThnAdminSsr(scope, environment, frontend, frontDoor, artifactBucket)
    : null;
  const pageSsrFunction = privateSsr?.ssrFunction || ssrFunction;
  const pageSsrOrigin = privateSsr?.ssrOrigin || ssrOrigin;
  ...
  pageSsrFunction.addPermission(`AllowCloudFrontInvokeFunctionUrl${idSuffix}`, {
    principal: new iam.ServicePrincipal("cloudfront.amazonaws.com"),
    action: "lambda:InvokeFunctionUrl",
    sourceArn: distributionArn,
    functionUrlAuthType: lambda.FunctionUrlAuthType.AWS_IAM,
  });
  pageSsrFunction.addPermission(`AllowCloudFrontInvokeFunction${idSuffix}`, {
    principal: new iam.ServicePrincipal("cloudfront.amazonaws.com"),
  ...
  const selectedStaticPaths = isThnAdminFrontDoor
    ...
    : staticPathPatterns();
  // Manifest viewer paths already contain /browser; the private-origin prefix
  // therefore ends at the immutable release, unlike the unchanged public origin.
  const selectedStaticOrigin = isThnAdminFrontDoor
    ? new origins.HttpOrigin(requiredValue(frontend.staticOriginDomainName, "staticOriginDomainName", environment), {
      originPath: `/${frontDoor.staticOriginPrefix}`,
      protocolPolicy: cloudfront.OriginProtocolPolicy.HTTPS_ONLY,
    })
    : staticOrigin;
  for (const configuredPath of selectedStaticPaths) {
    const pathPattern = isThnAdminFrontDoor
      ...
      : configuredPath;
    distribution.addBehavior(pathPattern, selectedStaticOrigin, {
      allowedMethods: isThnAdminFrontDoor
        ? cloudfront.AllowedMethods.ALLOW_ALL
        : cloudfront.AllowedMethods.ALLOW_GET_HEAD_OPTIONS,
      cachedMethods: cloudfront.CachedMethods.CACHE_GET_HEAD_OPTIONS,
...
function validateThnAdminFrontDoor(environment, frontend, frontDoor) {
  ...
  const selectedRelease = validateSelection(frontDoor.staticRelease);
  if (frontDoor.staticOriginPrefix !== selectedRelease.originPrefix
    || JSON.stringify(frontDoor.staticAssetPaths) !== JSON.stringify(selectedRelease.manifest.staticAssetPaths)) {
    throw new Error("THN admin static selection mismatch.");
  }
  ...
  const expectedBackendOwners = {
    "thn-admin-auth-runtime-v2": "thn-auth-runtime",
    "thn-admin-auth-v2": "thn-auth-admin",
  ...
  const expectedBackendOwnerSeals = {
    ...
    "thn-auth-runtime": "c51e8ef958d75b955b85e36a2a25a2cabff9d10bfc4abc6ffdcadb4b7c0abfe9",
    "auth-admin": "21af44b105a8f395e7c997c70342ad3222c7f3f1592f3b2d67381eda3e610091",
    "thn-auth-admin": "3f39ae0ff744c9411007ea2725599a3e1e98d64a4e629b2a4fb768fc14ba34a0",
  ...
  for (const backendRoute of backendRoutes) {
    ...
    const trustedOwner = trustedOwnerId === "thn-auth-runtime"
      ? { domainName: "5paiwwz4zl.execute-api.us-east-1.amazonaws.com", originPath: "/Prod" }
      : trustedOwnerId === "thn-auth-admin"
        ? { domainName: "d6h2nzsn0i.execute-api.us-east-1.amazonaws.com", originPath: "/test" }
        : (frontend.backendRoutes || []).find((candidate) => candidate.id === trustedOwnerId);
    if (
      !trustedOwner
...
function createBackendSamDeployRoles(scope, environment) {
  ...
  return services.map((service) => {
    ...
    if (service.repository === "zoolanding-config-authoring") {
      addCanonicalPolicy(scope, environment, "config", role.node.defaultChild);
      addCanonicalPolicy(scope, environment, "config-runtime", role.node.defaultChild);
    }
    return role;
  });
}
