...
function createFrontendDistribution(scope, props) {
  ...
  for (const backendRoute of frontend.backendRoutes || []) {
    const backendOrigin = new origins.HttpOrigin(resolveBackendRouteDomainName(scope, environment, backendRoute), {
...
function resolveBackendRouteDomainName(scope, environment, backendRoute) {
  const domainName = backendRoute.domainName;
  const apiIdParameterName = backendRoute.apiIdParameterName;

  if (domainName && apiIdParameterName) {
    throw new Error(
      `Backend route ${backendRoute.id || "unknown"} has ambiguous origin configuration for ${environment.name}.`
    );
  }
  if (domainName) {
    return requiredValue(domainName, "backendRoutes.domainName", environment);
  }
  if (environment.name !== "test" && environment.name !== "production") {
    throw new Error(`SSM-backed service API origins are not allowed for ${environment.name}.`);
  }

  const expectedParameterName = `/zoolanding/${environment.name}/services/${backendRoute.id}/api-id`;
  if (apiIdParameterName !== expectedParameterName) {
    throw new Error(
      `Invalid backend API id parameter for ${backendRoute.id || "unknown"} in ${environment.name}.`
    );
  }

  const apiId = ssm.StringParameter.valueForStringParameter(scope, apiIdParameterName);
  return cdk.Fn.join("", [
    apiId,
    `.execute-api.${environment.region}.`,
    cdk.Aws.URL_SUFFIX,
  ]);
}
