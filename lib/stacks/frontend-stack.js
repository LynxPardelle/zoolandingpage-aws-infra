...
function createBackendSamDeployRoles(scope, environment) {
  ...
  const services = [
    {
      id: "ConfigAuthoringTestDeployRole",
      roleName: "zoolanding-config-authoring-test-deploy",
      repository: "zoolanding-config-authoring",
      stackName: "zoolanding-config-authoring-test",
      buckets: ["zoolanding-config-payloads-test"],
      tables: ["zoolanding-config-registry-test"],
    },
    {
      id: "DataDropperTestDeployRole",
      roleName: "zoolanding-data-dropper-test-deploy",
      repository: "zoolanding-data-dropper-lambda",
      stackName: "zoolanding-data-dropper-test",
      buckets: ["zoolanding-data-raw-test"],
      tables: [],
    },
  ];
  ...
  return services.map((service) => {
    const role = new iam.Role(scope, service.id, {
      ...
      description: `GitHub OIDC SAM deploy role for ${service.repository} test.`,
      assumedBy: new iam.FederatedPrincipal(
        oidcProviderArn,
        {
          StringEquals: {
            "token.actions.githubusercontent.com:aud": "sts.amazonaws.com",
            "token.actions.githubusercontent.com:sub": `repo:LynxPardelle/${service.repository}:environment:test`,
          },
        },
        "sts:AssumeRoleWithWebIdentity"
      ),
    ...
    role.addToPolicy(new iam.PolicyStatement({
      actions: [
        ...
        "lambda:GetFunctionConfiguration", "lambda:ListTags", "lambda:RemovePermission", "lambda:TagResource",
        "lambda:UntagResource", "lambda:UpdateFunctionCode", "lambda:UpdateFunctionConfiguration",
      ],
      resources: [`arn:${cdk.Aws.PARTITION}:lambda:${environment.region}:${environment.account}:function:${service.stackName}-*`],
    }));
    role.addToPolicy(new iam.PolicyStatement({
    ...
    role.addToPolicy(new iam.PolicyStatement({
      ...
      resources: [`arn:${cdk.Aws.PARTITION}:iam::${environment.account}:role/${service.stackName}-*`],
    }));
    if (service.tables.length) {
      role.addToPolicy(new iam.PolicyStatement({
        actions: ["dynamodb:CreateTable", "dynamodb:DescribeTable", "dynamodb:TagResource", "dynamodb:UpdateTable"],
