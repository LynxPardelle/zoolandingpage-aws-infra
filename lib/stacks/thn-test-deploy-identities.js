...
const { addThnDedicatedRuntimeIdentity } = require("./thn-dedicated-runtime-identity");
...
function createThnTestDeployIdentities(scope, environment) {
  ...
  for (const [service, id] of [["api-proxy", "ApiProxy"], ["image-upload", "ImageUpload"]]) {
    ...
    if (service === "api-proxy") {
      ...
      addCanonicalPolicy(scope, environment, "api-runtime-corrected", github.node.defaultChild);
      addThnDedicatedRuntimeIdentity(scope, environment, github);
    }
